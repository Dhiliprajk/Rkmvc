...
function calculatePrice() {
    ...
    var total = (vehicle + ((vehicle * fuel) / 100) + ((vehicle * transmission) / 100)) * days*1000; // Formula
...
sendMail = () => {
  let usrObj = {
   const usrVehicle = document.getElementsByClassName("usrVehicle").value,
const rate = document.getElementsByClassName("rate").value,
...
const btn = document.getElementsByClassName("btn").value,
  ...
  usrObj.message.length > 0
    ? emailjs.send(`service_4m0ivbh`, "template_4byov9q", usrObj).then(
        function () {
          alert("We will be Reaching You Soon !");
        },
        function (error) {
          console.log("FAILED...", error);
          alert("Mail Not Recieved Try Again Some Time");
        }
      )
